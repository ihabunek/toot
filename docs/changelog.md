...
* TUI: preserve focused status when refreshing timeline (thanks @AbeEstrada)
...
* Rename `toot update_account` to `toot update-account`, the old name will still
  work.
* Rename `toot reblogged_by` to `toot reblogged-by`, the old name will still
  work.
