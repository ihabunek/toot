        ...
        if status.data["spoiler_text"] and not status.show_sensitive:
            ...
            if media:
                for m in media:
                    ...
                    if m["url"]:
# FIXME: uncomment this code to display previews of image media attachments
# after making the async_load_image code in app.py thread-safe.
#                         if m["url"].lower().endswith(('.jpg', '.jpeg', '.png', '.gif', '.svg')):
#                            yield urwid.Text("")
#                            try:
#                                ratio = float(m["meta"]["original"]["aspect"])
#                            except:
#                                ratio = 3/2 # pick something reasonable
#
